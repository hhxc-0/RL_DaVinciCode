...
class TestClass:
    ...
    def test_making_guess(self, setup_init, request):
        ...
        for player_index in range(0, number_players):
            with pytest.raises(ValueError): # test the exception with target index that is the player it self
        ...
        for player_comb in player_combs:
            ...
            with pytest.raises(ValueError): # test the exception with target index higher than bound
